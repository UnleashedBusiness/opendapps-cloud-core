...
abstract contract TreasuryBase is TreasuryInterface, ServiceDeployableInterface,
Initializable, ERC165Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable
{
    ...
    function __TreasuryBase__init(address _deployer, address pocketTemplate, address _controller) internal onlyInitializing {
        ...
        percents[_controller] = _percentMaxLocal();
        //100.0 %
    ...
    function _pendingBalance(address account, address token) internal view returns (uint256) {
        uint256 divisionPercent = _percentMaxLocal() == 0 ? 100 * LEGACY_PERCENT_SCALING : _percentMaxLocal();
